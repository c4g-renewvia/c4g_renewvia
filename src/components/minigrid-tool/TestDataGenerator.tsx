...
interface TestDataGeneratorProps {
  ...
  onCountChange: (_count: number) => void; // This is correct
  onGenerate: () => void; // No argument
  loading: boolean;
  error?: string | null;
}
  ...
  return (
    <div className='rounded-xl border border-zinc-700/70 bg-white p-6 backdrop-blur-sm dark:bg-zinc-900/50'>
      ...
      <div className='flex flex-wrap items-center gap-4'>
        <select
          value={selectedCount}
          onChange={(e) => onCountChange(Number(e.target.value))} // ← This uses the parameter correctly
